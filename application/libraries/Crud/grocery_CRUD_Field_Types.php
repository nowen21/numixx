...
class grocery_CRUD_Field_Types {
  ...
  public function get_field_types() {
    ...
    $types = array();
    // recorrer todos los campos de la tabla y verifica las propiedades del campo
    foreach ($this->basic_model->get_field_types_basic_table() as $field_info) { 
      // verifica que el campo requerido exista
      $field_info->required = !empty($this->required_fields) && in_array($field_info->name, $this->required_fields) ? 
              true : false;
      // formatea el texto que se va a motrar en la vista del campo
      $field_info->display_as = isset($this->display_as[$field_info->name]) ?
              $this->display_as[$field_info->name] : // si desde el controlador se le ha indicado como se debe mostrar
              ucfirst(str_replace("_", " ", $field_info->name)); // devuelve el nombre del campo de la tabla
